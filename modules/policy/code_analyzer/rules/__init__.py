...
class Rule_005c_EmotionRecognitionWorkplace(BaseRule):
    ...
    def check_ast(self, tree: ast.AST, source_lines: list[str], filename: str) -> Iterator[CodeFinding]:
        ...
        for node in ast.walk(tree):
            if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef, ast.ClassDef)):
                ...
                if self._EMOTION_PATTERNS.search(name) and self._CONTEXT_PATTERNS.search(name):
                    yield self._finding(filename, node.lineno,
...
class Rule_009a_NoRiskManagementSystem(BaseRule):
    ...
    _AI_MARKER   = re.compile(r"(llm|openai|anthropic|model\.predict|model\.generate|pipeline\.run|chat_complete)", re.I)

    def check_text(self, source: str, filename: str) -> Iterator[CodeFinding]:
        has_ai     = bool(self._AI_MARKER.search(source))
        has_risk   = bool(self._RISK_MARKER.search(source))
        if has_ai and not has_risk:
            yield self._finding(filename, 1,
...
class Rule_010b_NoBiasCheck(BaseRule):
    ...
    _BIAS_MARKERS   = re.compile(r"(fairness|bias|demographic_parity|equalized_odds|fairlearn|aif360|disparate_impact)", re.I)
...
class Rule_011a_NoDocstring(BaseRule):
    ...
    _AI_NAMES = re.compile(r"(LLM|Pipeline|Model|Classifier|Predictor|Detector|Recognizer|Ranker|Scorer)", re.I)
...
class Rule_013a_NoAIDisclosure(BaseRule):
    ...
    _RESPONSE_FN  = re.compile(r"def\s+(chat|respond|reply|generate_response|handle_message|send_response)", re.I)
    _DISCLOSURE   = re.compile(r"(AI system|generated by AI|automated|chatbot|virtual assistant|disclosure)", re.I)
...
class Rule_014b_NoConfidenceThreshold(BaseRule):
    ...
    _CONFIDENCE    = re.compile(r"(confidence|probability|score\s*[<>]=?\s*\d|threshold|certainty)", re.I)
...
class Rule_015a_NoInputValidation(BaseRule):
    ...
    _LLM_CALL    = re.compile(r"(messages\.create|chat\.completions|openai\.|anthropic\.|bedrock\.|pipeline\.run)", re.I)
    _VALIDATION  = re.compile(r"(validate|sanitize|guard|check_input|clean|strip|escape|guardrail|PromptInjection|PIIDetect)", re.I)
...
class Rule_015b_UnsafeDeserialization(BaseRule):
    ...
    _UNSAFE = re.compile(r"(pickle\.load|joblib\.load|torch\.load\s*\([^)]*(?!weights_only=True))", re.I)
            ...
            for pattern in self._PATTERNS:
                if pattern.search(line):
                    ...
                    if "os.getenv" in line or "os.environ" in line:
                        continue
                    yield self._finding(filename, i,
...
class Rule_050b_DeepfakeNoDisclosure(BaseRule):
    ...
    _PATTERNS = re.compile(r"(deepfake|face_swap|face_swap|voice_clone|voice_synthesis|synthetic_face)", re.I)
...
class Rule_GDPR_001_NoPurposeLimitation(BaseRule):
    ...
    def check_text(self, source: str, filename: str) -> Iterator[CodeFinding]:
        for match in self._PII_PASSED.finditer(source):
            ...
            yield self._finding(filename, line,
