...
def _find_env_file() -> Path:
    """
    Locate a `.env` by walking up from the working directory.

    The package used to derive this from its own location, which only worked
    from a source checkout. Under src/ layout -- and from an installed wheel --
    the module's location says nothing about the user's project root, so the
    search starts at the CWD and falls back to `./.env`.
    """
