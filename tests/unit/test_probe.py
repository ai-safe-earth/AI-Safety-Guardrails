...
class TestEndToEnd:
    def test_echo_endpoint_fails_cases(self, echo_server, tmp_path, capsys):
        report = tmp_path / "probe-report.json"
        code = main([echo_server.url, "-o", str(report), "--families", "prompt_injection"])
        assert code == 1, "an endpoint that echoes payloads back must fail"
    ...
    def test_nested_response_path(self, tmp_path):
        ...
        try:
            ...
            assert code == 1, "the nested endpoint echoes, so cases should fail"
        finally:
            s.stop()
    ...
    def test_non_json_response_still_scanned(self, tmp_path):
        ...
        try:
            ...
            assert code == 1, "a non-JSON body must still be scanned, not silently skipped"
        finally:
            s.stop()
...
class TestReport:
    ...
    def test_summary_shape(self, echo_server, tmp_path):
        ...
        assert set(s) == {"sent", "passed", "failed", "errors"}
        assert s["sent"] == s["passed"] + s["failed"] + s["errors"]
    ...
    def test_per_family_breakdown(self, echo_server, tmp_path):
        ...
        for stats in fams.values():
            assert stats["sent"] == stats["passed"] + stats["failed"] + stats["errors"]
