...
1. **Scope.** `ls docs/explain/` first: a topic continuing earlier work gets a NEW doc that
   ...
   Then read the project's roadmap/plan doc if one exists, and the commits involved
   (`git show --stat`).
...
- Don't restate what the project's state or plan docs already hold — cite them.
